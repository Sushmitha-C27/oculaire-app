# app.py — OCULAIRE Neon Lab v6 (safe-rerun patch)
...
# -----------------------
# Session state init + rerun guard init
# -----------------------
if "chat_history" not in st.session_state:
    st.session_state.chat_history = []
if "last_processed_q" not in st.session_state:
    st.session_state.last_processed_q = None
# guard to avoid double/unsafe experimental_rerun calls
if "_oculaire_rerun_guard" not in st.session_state:
    st.session_state["_oculaire_rerun_guard"] = False

def safe_rerun():
    """
    Try to call st.experimental_rerun() once in a guarded way.
    If it fails (Streamlit raising), swallow the exception so the app won't crash.
    We also set a guard flag so we don't attempt repeated reruns this execution.
    """
    # Only try once per call to avoid repeating unstable reruns
    if st.session_state.get("_oculaire_rerun_guard"):
        return
    st.session_state["_oculaire_rerun_guard"] = True
    try:
        st.experimental_rerun()
    except Exception as e:
        # fallback: don't crash — UI will reflect the updated session_state on next interaction
        # Keep a minimal debug message so developer logs show something
        st.error("⚠️ Rerun skipped (safe fallback). Interface will update on next interaction.")
        # Optional: write to logs (console)
        try:
            print("oculaire: safe_rerun caught exception:", repr(e))
        except Exception:
            pass
    ...
    if send_btn:
        if not user_question or user_question.strip() == "":
            ...
            if not API_KEY:
                ...
                # safe rerun to refresh UI (guarded)
                safe_rerun()
