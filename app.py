...
if analysis_trigger:
    ...
    if figs:
        ...
        st.session_state["pdf_bscan_conf"] = conf if 'conf' in locals() else None
...
def generate_full_pdf(figs,
    ...
    if rnflt_cluster == "Glaucoma-like" or bscan_label == "Glaucoma-like":
        ...
        risk_level = ("HIGH" if rnflt_severity >= 60 else
                      "MODERATE" if rnflt_severity >= 30 else
                      "LOW-MODERATE")
    else:
    ...
    exec_box = Paragraph(
        ...
        <b>Severity Index:</b> {rnflt_severity:.1f}%<br/>
        <b>CNN Confidence:</b> {bscan_conf:.1f}%<br/>
        """,
        body
    )
    ...
    if rnflt_cluster == "Glaucoma-like":
        findings = [
            ...
            f"- {rnflt_severity:.1f}% of retinal area flagged as at-risk.",
...
if figs:
    try:
        final_pdf = generate_full_pdf(
            ...
            bscan_conf=conf if 'conf' in locals() else 0
        )
